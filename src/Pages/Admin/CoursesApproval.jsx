...
export default function CourseApproval() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/course_gmt/courses/')
      .then((res) => res.json())
  ...
  const handleApprove = async (id) => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/course_gmt/update-course-status/', {
        method: 'PUT',
        body: formData,
      });
      if (response.ok) {
  ...
  const handleSubmitFeedback = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/student_gmt/send-feedback/', {
        method: 'POST',
